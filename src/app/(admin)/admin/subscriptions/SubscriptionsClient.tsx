...
import { PlanModal, type EditablePlan } from "./PlanModal";

interface PlanRow extends EditablePlan {
  activeSubCount: number;
}

export function SubscriptionsClient({ plans }: { plans: PlanRow[] }) {
  ...
  function openEdit(plan: EditablePlan) {
    setEditingPlan(plan);
    setModalOpen(true);
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <PlanModal key={editingPlan?.id ?? "new"} open={modalOpen} onOpenChange={setModalOpen} plan={editingPlan} />
    </div>
  );
}
